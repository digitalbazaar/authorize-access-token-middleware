...
export function authorizeAccessToken({
  ...
  return async (req, res, next) => {
    try {
      ...
      const {token: clientAccessToken} = auth;
      ...
      const {scope: tokenScope} = clientRegistration.client;

      if(!(typeof tokenScope === 'string' && tokenScope === scope)) {
...
async function _verifyJwt({jwt, validIssuers, tokenizer}) {
  let payload;
  try {
    const verifyFn = _createVerifyFn({tokenizer});
    ({payload} = await JWT.verify({jwt, verifyFn}));
...
function _createVerifyFn({tokenizer}) {
  return async function _verifyFn({
